...
locals {
  virtual_wan_hubs = {
    for virtual_hub_key, virtual_hub_value in var.virtual_wan_hubs : virtual_hub_key => merge(virtual_hub_value.hub, {
      location =  coalesce(virtual_hub_value.location, var.azure_location)
      # generated based on location
      resource_group_id = "${data.azurecaf_name.rg.result}-vwan-${virtual_hub_value.location}"

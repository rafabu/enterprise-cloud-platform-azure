...
locals {
  ...
  #     and details on shared key
  vpn_link_connection_helper = flatten([
    for k, v in local.vpn_connection_objects_hub_resolved_map : [
      for sc_key, sc_value in v.vpn_site_connections : {
        for link in sc_value.vpn_links : lower("${k}_${local.virtual_wan_hub_location_map[k].location}_${sc_key}_${link.name}") => {
          ecp_artefactName_hub             = k
          ecp_artefactName_site_connection = sc_key
          ecp_location                     = local.virtual_wan_hub_location_map[k].location

        
          ecp_hub_key = sc_value.hub_artefact_key == local.vwan_hub_artefact_default ? "ecpa_${lower(local.hub_locations[sc_value.location_key].azure_location)}_${local.virtual_hub_address_prefix_map["${sc_value.location_key}_${sc_value.hub_artefact_key}"].address_prefix}" : "${sc_value.location_key}_${sc_value.hub_artefact_key}_${local.virtual_hub_address_prefix_map[k].address_prefix}"

          ecp_hub_address_prefix = local.virtual_hub_address_prefix_map[k].address_prefix

          # vpn_gateway_name = provider::azapi::parse_resource_id(
          #   "Microsoft.Network/vpnGateways",
          #   data.azapi_resource.virtual_wan_hub_details[
          #     # k == local.vwan_hub_artefact_default ? "ecpa_${lower(local.virtual_wan_hub_location_map[k].location)}" : k
          #     sc_value.hub_artefact_key == local.vwan_hub_artefact_default ? "ecpa_${lower(local.hub_locations[sc_value.location_key].azure_location)}_${local.virtual_hub_address_prefix_map["${sc_value.location_key}_${sc_value.hub_artefact_key}"].address_prefix}" : "${sc_value.location_key}_${sc_value.hub_artefact_key}_${local.virtual_hub_address_prefix_map[k].address_prefix}"
          #   ].output.properties.vpnGateway.id
          # ).name

          #   ---> "ecpa_westeurope_10.224.0.0/23"
        }
      }
    ]
  ])
...
output "zzz_vpn_link_connection_helper" {
  value = local.vpn_link_connection_helper
}
...
# output "zzz_virtual_wan_hub_details" {
#   value = data.azapi_resource.virtual_wan_hub_details
# }

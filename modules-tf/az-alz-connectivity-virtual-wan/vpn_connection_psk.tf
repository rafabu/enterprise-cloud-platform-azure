    ...
    for k, v in local.vpn_connection_objects_hub_resolved_map : [
      for sc_key, sc_value in v.vpn_site_connections : {
        for link in sc_value.vpn_links : lower("${k}_${local.virtual_wan_hub_location_map[k].location}_${sc_key}_${link.name}") => {
          vpn_gateway_name         = sc_value.hub_artefact_key == local.vwan_hub_artefact_default ? "${replace(data.azurecaf_name.vwan.result, "-vwan-", "-vpng-")}-${lower(local.location_code[local.hub_locations[sc_value.location_key].azure_location])}-01" : "${replace(data.azurecaf_name.vwan.result, "-vwan-", "-vpng-")}-${lower(local.location_code[local.hub_locations[sc_value.location_key].azure_location])}-${format("%02d", random_integer.virtual_hub_id[k].result)}"
          ...
          id = provider::azapi::resource_group_resource_id(
            ...
            "${data.azurecaf_name.rg.result}-wan-${lower(local.location_code[local.hub_locations[sc_value.location_key].azure_location])}",
            "Microsoft.Network/vpnGateways/vpnConnections/vpnLinkConnections",
            [
              sc_value.hub_artefact_key == local.vwan_hub_artefact_default ? "${replace(data.azurecaf_name.vwan.result, "-vwan-", "-vpng-")}-${lower(local.location_code[local.hub_locations[sc_value.location_key].azure_location])}-01" : "${replace(data.azurecaf_name.vwan.result, "-vwan-", "-vpng-")}-${lower(local.location_code[local.hub_locations[sc_value.location_key].azure_location])}-${format("%02d", random_integer.virtual_hub_id[k].result)}",
              sc_value.name,
              link.name
            ]
          )
          ...
          shared_key_object = {
            ...
            value_key_vault_secret_name = lower(format(
              ...
              # remote site name
              local.vpn_site_objects_hub_resolved_map[k].vpn_sites[sc_value.site_artefact_key].name,

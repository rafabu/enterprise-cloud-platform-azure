locals {
  # security defaults must be disabled before conditional access policies can be applied
  policy_identitySecurityDefaultsEnforcement_enabled = length(var.named_location_definitions) > 0 || length(var.conditional_access_policy_definitions) > 0 ? false : true
...
resource "terraform_data" "policy_identity_security_defaults_enforcement_update" {
  ...
  provisioner "local-exec" {
    ...
    interpreter = ["pwsh", "-NoLogo", "-NonInteractive", "-ExecutionPolicy", "RemoteSigned", "-command"]
    command     = <<-SCRIPT
      az rest --method patch --url 'https://graph.microsoft.com/v1.0/policies/identitySecurityDefaultsEnforcementPolicy' --body '${jsonencode({ isEnabled = "true" })}' --headers 'Content-Type=application/json'
    SCRIPT
  }
}
...
# requires a PATCH operation / SINGLETON support - hence not yet supported
#
...
output "security_defaults_enforcement" {
  value = { isEnabled = data.msgraph_resource.policy_identity_security_defaults_enforcement.output.all.isEnabled }
}
